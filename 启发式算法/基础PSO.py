...
def VPUpdate(X, pop, c1, c2, Velocity, GbestPositon):
    X_new = copy.copy(X)
    ...
    for i in range(pop):
        for j in range(dim):
            ...
            Velocity_new[0, j] = Velocity[0, j] + c1 * np.random.randn() * (
...
def BorderCheck(X, ub, lb, pop, dim):
    for i in range(pop):
        for j in range(dim):
            if X[i, j] > ub[j]:
                X[i, j] = np.random.rand() * int(ub[j] - lb[j]) + lb[j]
            elif X[i, j] < lb[j]:
                X[i, j] = np.random.rand() * int(ub[j] - lb[j]) + lb[j]
    return X
...
def PSO(pop, dim, lb, ub, c1, c2, Velocity_max, Max_iter, fun):
    ...
    for i in range(Max_iter):
        X, Velocity = VPUpdate(X, pop, c1, c2, Velocity, GbestPositon)
...
# 适应度函数选择
fobj = fun
GbestScore, GbestPositon, Curve = PSO(pop, dim, lb, ub, c1, c2,
